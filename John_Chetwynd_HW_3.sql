...
SELECT
    CASE
        WHEN avg_vote >= 9.0 THEN 'Great'
        WHEN avg_vote < 9.0 AND avg_vote >= 8.0 THEN 'Good'
        WHEN avg_vote < 8.0 AND avg_vote >= 7.0 THEN 'Decent'
        WHEN avg_vote < 7.0 AND avg_vote >= 6.0 THEN 'Questionable'
...
SELECT
    production_company,
    SUM(CAST(LTRIM(usa_gross_income, '$') AS INTEGER)) AS total_usa_gross_income
FROM movies
 ...
 */
SELECT
        COUNT((avg_vote)) AS num_2008_movies_above_average
FROM movies
WHERE avg_vote > 6.4  AND  votes >=10000
    AND year = 2008
GROUP BY year;
...
SELECT
    ...
    ROUND(CAST(COUNT(CASE WHEN place_of_birth ILIKE '%California%' THEN place_of_birth END) AS NUMERIC)*100/CAST(COUNT(imdb_name_id) AS NUMERIC), 1) AS pct_california_born
FROM actors
 ...
 */
 SELECT
     CASE WHEN EXTRACT(dow FROM DATE(date_published))= (0) THEN 'Sunday'
        ...
        WHEN  EXTRACT(dow FROM DATE(date_published))= (2) THEN 'Tuseday'
        WHEN  EXTRACT(dow FROM DATE(date_published))= (3) THEN 'Wedsday'
        ...
        END AS day_of_week,
     COUNT(date_published) AS num_movies
FROM movies
